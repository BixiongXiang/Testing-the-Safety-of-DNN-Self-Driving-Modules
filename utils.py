...
def preprocess_image(img_path, target_size=(100, 100)):
    ...
    input_img_data = np.expand_dims(input_img_data, axis=0) # add a dimension at top [1,100,100,3]
    input_img_data = preprocess_input(input_img_data)  # transfer RGB image array to something
...
# my transform
def constraint_color(gradients):
    # they use an HSI color model, so we can change H so change the color
    new_grads = np.ones_like(gradients)
    # new_grads = np.where(gradients > 0, 0.2, 0.9) # generate something strange

    # grad_mean = 500 * np.max(gradients)
    # return grad_mean * new_grads
    # for i in range(0,100):
    #     for j in range(0,100):
    #         new_grads[0][i][j][0] = 1 * gradients[0][i][j][0]
    #         new_grads[0][i][j][1] = 1 * gradients[0][i][j][1]
    #         new_grads[0][i][j][2] = 1 * gradients[0][i][j][2]

    for i in range(0, 100):
        for j in range(0, 100):
            new_grads[0][i][j][0] = 1 * new_grads[0][i][j][0]  # -3.14
            new_grads[0][i][j][1] = 1 * new_grads[0][i][j][1]
            new_grads[0][i][j][2] = 1 * new_grads[0][i][j][2]
